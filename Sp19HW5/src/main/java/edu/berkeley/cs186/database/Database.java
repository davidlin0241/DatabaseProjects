...
public class Database {
    ...
    public class Transaction extends BaseTransaction {
        ...
        public void end() {
            ...
            // TODO(hw5_part2): release all locks (in bottom up order)
            List<Lock> tLocks = lockManager.getLocks(this);
            List<LockContext> lockContexts = new ArrayList<>();


            for (int i = 0; i < tLocks.size(); i++) {
                lockContexts.add(LockContext.fromResourceName(lockManager, tLocks.get(i).name));
            }
        ...
        public Iterator<Record> sortedScan(String tableName, String columnName) throws DatabaseException {
            ...
            LockUtil.ensureSufficientLockHeld(this, getTableContext(tableName), LockType.S);//OK
            try {
