...
public class QueryPlan {
    ...
    private HashSet<String> createHashSet(String s) {
     ...
     * @throws QueryPlanException
     */
    //SEE 4TH POST OF PIAZZA
    //NO CALLING ADDSELECTS. DO NOT CONSIDER INTERESTING ORDER
    public Iterator<Record> executeOptimal() throws DatabaseException, QueryPlanException {
        //TODO: HW4 Implement

        Map<Set, QueryOperator> m1 = new HashMap<>(), m2;
        ...
        m1.put(createHashSet(startTableName), minCostSingleAccess(startTableName));
        //System.out.println(m1);
        for (String tName: joinTableNames) {
            m1.put(createHashSet(tName), minCostSingleAccess(tName));
        ...
        */
        ...
        m2 = m1;
        while (m2.size() > 1) {
            m2 = minCostJoins(m2, m1);
        }
        //should only be going down by one each iteration..
        ...
        finalOperator = minCostOperator(m2);
        addGroupBy();
        addProjects();
    ...
    public QueryOperator minCostSingleAccess(String table) throws DatabaseException,
        ...
        for (int i: getEligibleIndexColumns(table)) {
            op = new IndexScanOperator(transaction,
                    table,
                    selectColumnNames.get(i),
                    selectOperators.get(i),
    ...
    private Set getKey(Map<Set, QueryOperator> pass1Map, String name) {
        Iterator<String> setIter;
        String s;
        for (Set<String> table: pass1Map.keySet()) {
            setIter = table.iterator();
            s = setIter.next();
            if (s.equals(name)) {
                return table;
            }
        }
        return null; //will never happen (i think).
    ...
    private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        String [] left, right;
        String leftName, leftCol, rightName, rightCol;
        QueryOperator op, op2, minOp;
        Set<String> key, newTables;
        ...
        for (Set tables: prevMap.keySet()) {
            for (int i = 0; i < joinTableNames.size(); i++) {
                ...
                */
                ...
                op = prevMap.get(tables);
                newTables = new HashSet<>();

                if (tables.contains(leftName) && !tables.contains(rightName)) {
                    key = getKey(pass1Map, rightName);
                    op2 = pass1Map.get(key);
                    minOp = minCostJoinType(op, op2, leftCol, rightCol);
                } else if (!tables.contains(leftName) && tables.contains(rightName)) {
                    key = getKey(pass1Map, leftName);
                    op2 = pass1Map.get(key);
                    minOp = minCostJoinType(op2, op, leftCol, rightCol);
                } else {
                    continue;
                }

                newTables.addAll(tables);
                newTables.addAll(key);
                newCost = minOp.estimateIOCost();

                if (map.containsKey(newTables)) {
                    currCost = map.get(newTables).estimateIOCost();
                } else {
                    currCost = Integer.MAX_VALUE;
                }

                if (newCost < currCost) {
                    map.put(newTables, minOp);
                }
            }
        }
