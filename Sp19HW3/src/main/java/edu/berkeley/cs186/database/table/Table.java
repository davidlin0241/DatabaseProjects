...
public class Table implements Closeable {
    ...
    public class RIDPageIterator implements BacktrackingIterator<RecordId> {
        ...
        private int currIndex = 0, markedIndex = -1;
        ...
        */
        ...
        public boolean hasNext() {
            //throw new UnsupportedOperationException("TODO(hw3): implement");
            return currIndex < rids.size();
        }

        //w Don't assume hasNext() is always called before next()
        public RecordId next() {
            if (!hasNext()) {
                throw new NoSuchElementException("No more elements.");
            }
            return rids.get(currIndex++);
            //throw new UnsupportedOperationException("TODO(hw3): implement");
        }

        public void mark() {
            //throw new UnsupportedOperationException("TODO(hw3): implement");
            if (this.currIndex == 0 || this.currIndex < this.markedIndex) {
                return;
            }
            this.markedIndex = this.currIndex;
        }
        ...
        public void reset() {
            ...
            this.currIndex = this.markedIndex - 1;
        }
    }
    ...
    public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
        ...
        public boolean hasNext() {
            ...
            if (pageIter.hasNext()) {
                ...
                return hasNext(); //in case current recordIter has no records but there's still page ahead that has
            }
