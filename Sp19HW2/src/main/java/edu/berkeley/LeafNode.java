...
class LeafNode extends BPlusNode {
    ...
    throws BPlusTreeException {
        ...
        while (keys.size() > d) {
            ...
            rightRids.add(rids.remove(d));
        }

        LeafNode rightSib = new LeafNode(metadata, rightKeys, rightRids,
                this.rightSibling, transaction);

        this.rightSibling = Optional.of(rightSib.getPage().getPageNum());
        sync(transaction);
        return Optional.of(new Pair<DataBox, Integer>(rightKeys.get(0), rightSib.getPage().getPageNum()));
    ...
    public Optional<Pair<DataBox, Integer>> bulkLoad(BaseTransaction transaction,
            Iterator<Pair<DataBox, RecordId>> data, float fillFactor) throws BPlusTreeException {
        //throw new UnsupportedOperationException("TODO(hw2): implement");
        ...
        if (keys.size() > fillLimit) {
            ...
            LeafNode rightSib = new LeafNode(metadata, rightKeys, rightRids,
                    this.rightSibling, transaction);
            this.rightSibling = Optional.of(rightSib.getPage().getPageNum());
            //System.out.println(this.toSexp(transaction));
            sync(transaction);
            return Optional.of(new Pair<DataBox, Integer>(rightKeys.get(0), rightSib.getPage().getPageNum()));
        }
