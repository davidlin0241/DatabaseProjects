...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(BaseTransaction transaction, DataBox key) {
        ...
        BPlusNode child = BPlusNode.fromBytes(transaction, metadata, children.get(index));

        while (child instanceof InnerNode) {
            child = child.get(transaction, key);
        }
    ...
    public LeafNode getLeftmostLeaf(BaseTransaction transaction) {
        ...
        while (child instanceof InnerNode) {
            child = child.getLeftmostLeaf(transaction);
        }
    ...
    throws BPlusTreeException {
        ...
        int index = numLessThanEqual(key, keys);
        BPlusNode child = getChild(transaction, index);
        Optional<Pair<DataBox, Integer>> pair = child.put(transaction, key, rid);
        //above recurses until reaching leaf

        if (pair.isPresent()) {

            DataBox middleKey = pair.get().getFirst();
            Integer rightPageNum = pair.get().getSecond();
            int i = InnerNode.numLessThanEqual(middleKey, keys);

            keys.add(i, middleKey);
            children.add(i + 1, rightPageNum);
            ...
            if (keys.size() > metadata.getOrder() * 2) {
                ...
                while (keys.size() > d + 1) {
                    rightKeys.add(keys.remove(d + 1));
                    rightChildren.add(children.remove(d + 1));
                }

                DataBox midKey = keys.remove(d);
                InnerNode rightNode = new InnerNode(metadata, rightKeys, rightChildren, transaction);
                sync(transaction);
                //System.out.println(rightNode.toSexp(null));
    ...
    throws BPlusTreeException {
        //throw new UnsupportedOperationException("TODO(hw2): implement");
        BPlusNode rightChild = null;
        Optional<Pair<DataBox, Integer>> p;
        while (keys.size() <= 2 * metadata.getOrder() && data.hasNext()) {
            rightChild = getChild(transaction, children.size() - 1);
            p = rightChild.bulkLoad(transaction, data, fillFactor);

            if (p.isPresent()) {
                keys.add(p.get().getFirst());
                children.add(p.get().getSecond());
            }
        }
        
        //System.out.println(rightChild.toSexp(transaction));
        ...
        if (keys.size() > metadata.getOrder() * 2) {
           ...
           // System.out.println(rightNode.toSexp(transaction));
            return Optional.of(new Pair<>(midKey, rightNode.getPage().getPageNum()));
        }
    ...
    public void remove(BaseTransaction transaction, DataBox key) {
        ...
        LeafNode leaf = get(transaction, key);
