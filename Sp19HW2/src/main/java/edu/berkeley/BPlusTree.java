 ...
 *   BPlusTree fromDisk = new x Tree("tree.txt");
 *
...
public class BPlusTree implements Closeable {
    ...
    public void put(BaseTransaction transaction, DataBox key, RecordId rid) throws BPlusTreeException {
        typecheck(key);
        //throw new UnsupportedOperationException("TODO(hw2): implement");
        Optional<Pair<DataBox, Integer>> p = root.put(transaction, key, rid);

        if (p.isPresent()) {
            DataBox d = p.get().getFirst();
            int pg = p.get().getSecond();
    ...
    public void bulkLoad(BaseTransaction transaction, Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (scanAll(transaction).hasNext()) {
            ...
            while (data.hasNext()) {
                Optional<Pair<DataBox, Integer>> p = root.bulkLoad(transaction, data, fillFactor);

                if (p.isPresent()) {
                    List<DataBox> rightKeys = new ArrayList<>();
                    List<Integer> rightChildren = new ArrayList<>();

                    rightKeys.add(p.get().getFirst());
                    rightChildren.add(root.getPage().getPageNum());
                    rightChildren.add(p.get().getSecond());

                    //System.out.println(root.toSexp(transaction));
                    root = new InnerNode(metadata, rightKeys, rightChildren, transaction);
                    //System.out.println(root.toSexp(transaction));
                }
            }
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode leaf;
        Optional<LeafNode> rightSib;
        BaseTransaction trans;
        Iterator<RecordId> ridIter;


        public BPlusTreeIterator(BaseTransaction transaction) {
            leaf = root.getLeftmostLeaf(transaction);
            trans = transaction;
            ridIter = leaf.scanAll();
        }

        public BPlusTreeIterator(BaseTransaction transaction, DataBox key) {
            leaf = root.get(transaction, key);
            trans = transaction;
            ridIter = leaf.scanGreaterEqual(key);
        }

        @Override
        public boolean hasNext() {
            //throw new UnsupportedOperationException("TODO(hw2): implement");
            rightSib = leaf.getRightSibling(trans);
            if (ridIter.hasNext() || rightSib.isPresent()) {
                return true;
            } else {
                return false;
            }
