...
public class TestLeafNode {
    ...
    public void testPutOverflow() throws BPlusTreeException, IOException {
        int d = 1;
        //List<Optional<Pair<DataBox, Integer>>> l = new ArrayList<>();
        ...
        assertEquals(Optional.of(new RecordId(0, (short) 0)), leaf.getKey(new IntDataBox(0)));
        assertEquals(Optional.empty(), leaf.getKey(new IntDataBox(1)));

        Optional<LeafNode> rightSib = leaf.getRightSibling(null);
        DataBox key1 = new IntDataBox(1), key2 = new IntDataBox(2);
        assertEquals(Optional.of(new RecordId(1, (short) 1)), rightSib.get().getKey(key1));
        assertEquals(Optional.of(new RecordId(2, (short) 2)), rightSib.get().getKey(key2));
    }
